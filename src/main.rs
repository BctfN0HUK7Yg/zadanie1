    ...
    R: Clone,
{
    ...
    for i in (0..work.len()).step_by(TRASHHOLD) {
        let size_of_work = if i + TRASHHOLD > work.len() {
            work.len() - i
        } else {
            TRASHHOLD
        }; //last part should be less then TRASHHOLD

        ranges_of_work.push((i, i + size_of_work));
    }
    let num_of_threasds = if work.len() % TRASHHOLD > 0 {
        work.len() / TRASHHOLD + 1
    } else {
        work.len() / TRASHHOLD
    };
    ...
    crossbeam::scope(|s| {
        ...
        for y in 0..num_of_threasds {
            ...
            println!(
                "Start thread {} to compute work range from {} to {}",
                y, current_range.0, current_range.1
            );
            let handle = s.spawn(move |_| {
                let mut res: Vec<R> = Vec::new();

                for i in 0..part_of_work.len() {
                    println!("\tdoing work in thread {:?} with element {}", y, i);
